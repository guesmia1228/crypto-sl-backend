...
public class AdminDashboardController {
    ...
    @PostMapping("/users")
    public ResponseEntity<?> getClicks(@RequestBody AddUserRequest addUserRequest) throws UserAlreadyExistsException {
        log.info("Admin process to add new user! ");
        return ResponseEntity.ok(userService.addUser(addUserRequest));
    }

    @PatchMapping("/users")
    public ResponseEntity<?> changeState(@RequestBody ChangeUserStateRequest changeUserStateRequest) throws UserNotFoundException {

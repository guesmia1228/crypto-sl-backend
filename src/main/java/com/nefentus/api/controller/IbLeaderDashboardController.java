...
@RestController
@RequestMapping("/api/dashboard/ibleader")
@PreAuthorize("hasRole('IB_LEADER')")
@CrossOrigin(origins = "*", maxAge = 3600)
@AllArgsConstructor
@Slf4j
public class IbLeaderDashboardController {
    ...
    public ResponseEntity<?> checkPermission() {
        log.info("Check IbLeader user permission! ");
    ...
    public ResponseEntity<?> getTotalIncome(Principal principal) throws UserNotFoundException {
        log.info("IbLeader user request to get total income with email= {}", principal.getName());
    ...
    public ResponseEntity<?> getTotalIncomesPerDay(Principal principal) throws UserNotFoundException {
        log.info("IbLeader user request to get total income per day with email= {}", principal.getName());
    ...
    public ResponseEntity<?> getTotalUserCount(Principal principal) {
        log.info("IbLeader user request to count total users with email= {}", principal.getName());
    ...
    public ResponseEntity<?> getAllUsers(Principal principal) {
        log.info("IbLeader user request to get all users with email= {}", principal.getName());
    ...
    public ResponseEntity<?> getClicks(Principal principal) {
        log.info("IbLeader user request to count total clicks with email= {}", principal.getName());
    ...
    public ResponseEntity<?> getClicks(@RequestBody AddUserRequest addUserRequest, Principal principal) throws UserAlreadyExistsException {
        log.info("IbLeader user= {} request to add new user! ", principal.getName());
